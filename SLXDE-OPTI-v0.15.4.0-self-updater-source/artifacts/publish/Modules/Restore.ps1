...
function Restore-WindowedOptimizationsDefault {
    try {
        Write-Log "Restoring Windowed Optimisations to Windows default"
    ...
    catch { Write-Log "Restore Windowed Optimisations failed: $($_.Exception.Message)" "ERROR"; return "Failed" }
}
...
function Restore-GamingTweaks {
    ...
    Write-Status "Restore Windowed Optimisations" $Windowed $(if ($Windowed -eq "Success") { "Green" } else { "Red" })
