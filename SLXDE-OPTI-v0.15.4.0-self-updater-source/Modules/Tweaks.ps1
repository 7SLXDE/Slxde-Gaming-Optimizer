...
function Enable-WindowedOptimizations {
    try {
        ...
        Write-Log "Windowed optimisations enabled"
        return "Success"
    }
    catch {
        Write-Log "Failed to enable windowed optimisations: $($_.Exception.Message)" "ERROR"
        return "Failed"
    }
}
...
function Disable-FullscreenOptimizationsForExe {
    Show-Banner
    Write-Host "Disable Fullscreen Optimisations for Game EXE" -ForegroundColor Yellow
    Write-Host ""
    ...
    try {
        ...
        Write-Log "Disabled fullscreen optimisations for $Path"
        Write-Host "Fullscreen optimisations disabled for:" -ForegroundColor Green
        Write-Host $Path
    }
    catch {
        Write-Log "Failed to disable fullscreen optimisations: $($_.Exception.Message)" "ERROR"
...
function Invoke-SafeGamingTweaks {
    ...
    Write-Status "Windowed Optimisations" (Enable-WindowedOptimizations) "Green"
...
function Restore-GamingTweaks {
    ...
    try {
        ...
        Write-Status "Restore Windowed Optimisations" "Success" "Green"
    } catch { Write-Status "Restore Windowed Optimisations" "Failed" "Red" }
...
function Show-GamingTweaksMenu {
    while ($true) {
        ...
        Write-Host "  4. Enable Windowed Optimisations"
        Write-Host "  5. Disable Mouse Acceleration"
        Write-Host "  6. Disable Fullscreen Optimisations for selected EXE"
        ...
        switch ($Choice.ToUpper()) {
            ...
            "4" { Write-Status "Windowed Optimisations" (Enable-WindowedOptimizations) "Green"; Pause-App }
